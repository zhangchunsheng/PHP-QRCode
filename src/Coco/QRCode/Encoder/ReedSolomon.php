...
class ReedSolomon {
    ...
    function __construct(array $dataCode, int $dataLength, int $b1, int $b2, int $blocks,int $nroots) {
        ...
        if($this->nroots >= 256){
            throw \Coco\QRCode\qrException::EncoderError("version estimation failed");
        }
        ...
        if($sr != 1) {
            throw \Coco\QRCode\qrException::EncoderError('field generator polynomial is not primitive!');
        }
